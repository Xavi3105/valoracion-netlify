exports.handler = async function(event) {
  ...
  const BASE = 'https://www.catastro.hacienda.gob.es/OVCServWeb/OVCWcfCallejero/COVCCallejero.svc/json';

  let url;

  if (rc) {
    const rcClean = rc.trim().replace(/\s/g, '').toUpperCase();
    url = `${BASE}/ConsultaDNPRC?RefCat=${encodeURIComponent(rcClean)}`;
  } else if (provincia && municipio && nomvia && numero) {
    const tv = tipovia || 'CL';
    let locUrl = `${BASE}/ConsultaDNPLOC?Provincia=${encodeURIComponent(provincia)}&Municipio=${encodeURIComponent(municipio)}&TipoVia=${encodeURIComponent(tv)}&NomVia=${encodeURIComponent(nomvia)}&Numero=${encodeURIComponent(numero)}`;
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ error: `Catastro respondió con error ${response.status}. Comprueba que la referencia catastral es correcta.` })
      };
    }
    ...
    try {
      ...
      return {
        statusCode: 500,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ error: 'Catastro devolvió una respuesta inesperada.' })
      };
    }
